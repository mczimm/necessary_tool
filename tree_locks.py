...
def sql_stat(conn,sql_id,total):
  ...
  try:
	...
	where sql_id = '%s'
	order by 1 desc) where rownum <= 300
      """
...
def kill_sess_by_sid(conn,sid,ser,inst):
  try:
    SQL_BIND ="""
declare
begin
    execute immediate 'alter system kill session ''%s,%s,@%s''';
end;
    """

    SQL=SQL_BIND% (sid,ser,inst)
...
def sql_patch_create(conn,sql_id,hint,desc):
  try:
    SQL_BIND ="""
	begin
for reco in (select sql_fulltext from gv$sqlarea where sql_id = '%s')
loop
...
def sql_profile_drop(conn,name):
  try:
    SQL_BIND ="""
        begin
sys.DBMS_SQLTUNE.DROP_SQL_PROFILE('%s');
end;
    """
...
def top_event_between_snap(conn,sql_id,snap_begin,snap_end):
  try:
    SQL_BIND ="""
select * from (select count(*),event from dba_hist_active_sess_history where sql_id = '%s' and snap_id between %s and %s group by event order by 1 desc) where rownum <= 100
    """

    SQL=SQL_BIND% (sql_id,snap_begin,snap_end)
    ...
    headers = [term.blue+"COUNT","EVENT"+term.normal]
...
select index_owner,table_name,column_name,descend from dba_ind_columns where index_name = upper('%s') or index_name = '%s'
    ...
    headers = [term.blue+"OWNER","TAB_NAME","COL_NAME","DESCEND"+term.normal]
...
def report_col_usage(conn,owner,tab):
  try:
    str_out=""
    SQL_BIND ="""
	select dbms_stats.report_col_usage('%s', '%s') from dual
    """

    SQL=SQL_BIND% (owner,tab)
...
def ctas_ash(conn):
  try:
    ...
    tm=tm_year+""+tm_mon+""+tm_mday+"_"+tm_hour+""+tm_min+""+tm_sec

    str_t="system.ash_"
    str_t+=tm
    SQL_BIND ="""
        create table %s as select * from gv$active_session_history
    """
    ...
    cur.execute(SQL)
    curr_time()
    print 'Table %s has been created' % str_t

  except cx_Oracle.DatabaseError,info:
    print "Error: ",info

def tune_task_create(conn,sql_id):
  try:
    SQL_BIND ="""
DECLARE
...
END;
    ...
    sql2 = sql_id
    sql3 = sql_id
    SQL=SQL_BIND% (sql1,sql2,sql3)

    print 'Creating sql tuning task has been started. It will take a while, but no more than 20 min.\n'
...
def tune_task_drop(conn,task_name):
  try:
    SQL_BIND ="""
BEGIN
...
def lc_obj(conn,obj):
  try:
    SQL_BIND ="""
select b.inst_id,b.sid,b.serial#,c.owner,c.object_name,c.object_type,b.status,a.locked_mode,b.osuser,b.machine,b.client_identifier
from gv$locked_object a,gv$session b,dba_objects c
where b.sid = a.session_id and a.object_id = c.object_id
and upper(c.OBJECT_NAME) like upper('%%%s%%')
order by b.status
    """
    SQL=SQL_BIND% (obj)
...
def oebs_pending_hold(conn):
  try:
    SQL_BIND ="""
SELECT request_id id, nvl(meaning, 'UNKNOWN') status, user_concurrent_program_name, concurrent_program_name,
to_char(request_date, 'DD-MON-RR HH24:MI:SS') submitd, fcr.last_update_date, 
fu1.user_name as "req_name", fu2.user_name as "ch_name"
FROM APPS.fnd_concurrent_requests fcr, APPS.fnd_lookups fl, APPS.fnd_concurrent_programs_vl fcpv, apps.fnd_user fu1, apps.fnd_user fu2
WHERE phase_code = 'P' AND hold_flag = 'Y' AND fcr.requested_start_date >= TO_CHAR(ADD_MONTHS(sysdate,-1))
...
AND fcr.requested_by = fu1.user_id AND fcr.last_updated_by = fu2.user_id
ORDER BY request_date, request_id
    ...
    for a in range(len(res)):
        ...
        for b in range(len(res[a])):
                ...
                #if match:
                  #str_chk = term.red+str_chk+term.normal
                a_list.append(str_chk.decode('utf8'))
        table_list.append(a_list)

    headers = [term.blue+"ID","STATUS","USER_PROG_NAME","SYS_PROG_NAME","SUBMIT","LAST_UPD_DT","USER_REQ","USER_CHANGED"+term.normal]
    curr_time()
    print tabulate(table_list,headers,tablefmt="plain")
...
def oebs_pending_non_hold(conn):
  try:
    SQL_BIND ="""
select * from (
SELECT request_id id, nvl(meaning, 'UNKNOWN') status, user_concurrent_program_name pname,
to_char(request_date, 'DD-MON-RR HH24:MI:SS') submited, to_char(requested_start_date, 'DD-MON-RR HH24:MI:SS') requestd, fu2.user_name as "ch_name"
FROM APPS.fnd_concurrent_requests fcr, APPS.fnd_lookups fl, APPS.fnd_concurrent_programs_vl fcpv, apps.fnd_user fu2
WHERE phase_code = 'P' AND hold_flag = 'N' AND fcr.requested_start_date >= TO_CHAR(ADD_MONTHS(sysdate,-1))
AND status_code != 'P' AND LOOKUP_TYPE = 'CP_STATUS_CODE' AND lookup_code = status_code
AND fcr.concurrent_program_id = fcpv.concurrent_program_id AND fcr.program_application_id = fcpv.application_id
AND fcr.last_updated_by = fu2.user_id
ORDER BY request_date, request_id) where rownum <=100
    """
    SQL=SQL_BIND% ()
    ...
    headers = [term.blue+"ID","STATUS","USER_PROG_NAME","SUBMITED","REQUESTED","USER_CHANGED"+term.normal]
    curr_time()
    print tabulate(table_list,headers,tablefmt="plain")
...
try:
	...
	if sys.argv[1] == '-sql_profile_drop':
		conn = connection()
		sql_profile_drop(conn,sys.argv[2])
	...
	if sys.argv[1] == '-top_event_for_sql':
		conn = connection()
		top_event_between_snap(conn,sys.argv[2],sys.argv[3],sys.argv[4])
	if sys.argv[1] == '-ind_col':
		conn = connection()
		ind_col(conn,sys.argv[2])
	if sys.argv[1] == '-report_col_usage':
		conn = connection()
		report_col_usage(conn,sys.argv[2],sys.argv[3])
	if sys.argv[1] == '-ctas_ash':
		conn = connection()
		ctas_ash(conn)
	if sys.argv[1] == '-lc_obj':
		conn = connection()
